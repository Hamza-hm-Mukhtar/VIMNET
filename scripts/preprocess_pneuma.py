...
from vimnet.data.preprocess_pneuma import PNeumaPreprocessConfig, preprocess_pneuma
from vimnet.data.stsg import NeighborhoodConfig


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("--cfg", type=str, default="configs/preprocess/pneuma.yaml")
    args = ap.parse_args()
    ...
    cfg = PNeumaPreprocessConfig(
        ...
        coords=str(cfg_dict.get("coords", "auto")),
        include_types=cfg_dict.get("include_types", None),
    )

    splits_yaml = Path(cfg_dict.get("splits_yaml", "configs/splits/pneuma.yaml"))
    splits = yaml.safe_load(splits_yaml.read_text()) or {}
    preprocess_pneuma(cfg, split_files={"train": splits.get("train", []), "val": splits.get("val", []), "test": splits.get("test", [])})
